...
def cox_partial_likehood_loss(score, Y_hazard, Y_survival, epsilon=1e-8):
    ...
    for j in range(len(t)):
        total_1 = np.sum(np.log(score[H[j]] + epsilon))
        m = n[j]
        total_2 = 0.0
        for i in range(m):
...
def isotonic_rank_loss(score, Y_hazard, Y_survival, num_time_units, score_prev_layer):
    ...
    for i, j in acc_pair:        
        if Y_hazard[j] > num_time_units * 30:
            yj = num_time_units + 1
        else:
            yj = np.ceil(Y_hazard[j] / 30)


        yi = np.ceil(Y_hazard[i] / 30)

        rank_loss = yj - yi

        regression_loss = np.linalg.norm(score[j] - score[i] - 1)
        total.append(regression_loss * rank_loss)
...
def isotonic_rank_loss_grad_impl(score, Y_hazard, Y_survival, num_time_units, score_prev_layer, grad):
    ...
    for i,j in acc_pair: #Change to j,i instead of i,j in original code
        ...
        L2dist = np.linalg.norm(score[j] - score[i] - 1)
        tmp1 = np.zeros(score[j].shape, dtype=np.float64)
        tmp2 = np.zeros(score[i].shape, dtype=np.float64)
        
        tmp1 = (-1 * score_prev_layer[j]) * np.multiply(score[j], 1 - score[j])
        tmp2 = (-1 * score_prev_layer[i]) * np.multiply(score[i], 1 - score[i])

        tmp = tmp1 - tmp2
        l1 = score[j] - score[i] - 1
        diff_value = np.dot(l1, tmp)
        total = total + (yj-yi) * diff_value / L2dist
...
def train(X_train, Y_hazard_train, Y_survival_train, BATCH_SIZE=32, learning_rate=0.001, epochs=1200, display_rate=1):
...
def test(sess, X_test, Y_hazard_test, Y_survival_test):
    ...
    score1, score2 = model(X, HIDDEN_UNITS, TIME_UNITS, DROPOUT_PROB_TEST, batch_norm_epsilon=1e-7, train=False, reuse=True)

    result_score1, result_score2 = sess.run([score1, score2], feed_dict = {X: X_test})
...
def main():
    ...
    score1, score2 = test(sess, X_test, Y_test[:, 1], Y_test[:, 0])
